...
import { AxesAndRulesDefaults, LineDefaults, chartTypes, yAxisSides } from '../utils/constants';
import { BarAndLineChartsWrapperTypes, HorizSectionsType, RuleType } from '../utils/types';
...
export const LineChartBicolor = (props: propTypes) => {
    ...
    const spacing =
        props.spacing ??
        (adjustToWidth
            ? ((props.width || AxesAndRulesDefaults.width) - initialSpacing) / data.length
            : LineDefaults.spacing);

    const xAxisThickness = props.xAxisThickness ?? AxesAndRulesDefaults.xAxisThickness;
    const dataPointsHeight1 = props.dataPointsHeight ?? LineDefaults.dataPointsHeight;
    const dataPointsWidth1 = props.dataPointsWidth ?? LineDefaults.dataPointsWidth;
    const dataPointsRadius1 = props.dataPointsRadius ?? LineDefaults.dataPointsRadius;
    ...
    const getY = (index) => yAtxAxis - (data[index].value * containerHeight) / maxValue;

    useEffect(() => {
        const ppArray: Array<Points> = [];
        ...
        pointsArray.forEach((item: any, index) => {
            const splitArray = item.points.split(' ').filter((spItem) => spItem && spItem !== ' ');
    ...
    const startOpacityNegative = props.startOpacityNegative ?? LineDefaults.startOpacity;
                    ...
                    <Rect
                        ...
                        y={
                            item.verticalLineUptoDataPoint
                                ? containerHeight - (item.value * containerHeight) / maxValue + 10
                                : -xAxisThickness
                        }
