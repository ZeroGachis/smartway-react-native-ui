...
export const BarChart = (props: BarChartPropsType) => {
    ...
    const animationDuration = props.animationDuration ?? BarDefaults.animationDuration;
    ...
    const lineConfig = props.lineConfig
        ? {
              initialSpacing: props.lineConfig.initialSpacing ?? defaultLineConfig.initialSpacing,
              ...
              animationDuration:
                  props.lineConfig.animationDuration || defaultLineConfig.animationDuration,
              ...
              dataPointsHeight:
                  props.lineConfig.dataPointsHeight || defaultLineConfig.dataPointsHeight,
              dataPointsWidth:
              ...
              dataPointsRadius:
                  props.lineConfig.dataPointsRadius || defaultLineConfig.dataPointsRadius,
              ...
              arrowConfig: {
                  length:
                      props.lineConfig.arrowConfig?.length ?? defaultLineConfig.arrowConfig?.length,
                  width:
                      props.lineConfig.arrowConfig?.width ?? defaultLineConfig.arrowConfig?.width,
    ...
    const labelsExtraHeight = props.labelsExtraHeight ?? AxesAndRulesDefaults.labelsExtraHeight;
    ...
    if (props.stackData) {
        props.stackData.forEach((stackItem) => {
            let stackSum = stackItem.stacks.reduce((acc, stack) => acc + (stack.value ?? 0), 0);
            if (stackSum > maxItem) {
                maxItem = stackSum;
            }
            if (stackSum < minItem) {
            ...
            totalWidth +=
                (stackItem.stacks[0].barWidth ?? props.barWidth ?? BarDefaults.barWidth) + spacing;
        });
    } else {
    ...
    const showScrollIndicator = props.showScrollIndicator ?? BarDefaults.showScrollIndicator;
    ...
    if (noOfSectionsBelowXAxis) {
        for (let i = 1; i <= noOfSectionsBelowXAxis; i++) {
            ...
            if (showFractionalValues || props.roundToDigits) {
                value = parseFloat(
                    value.toFixed(props.roundToDigits ?? AxesAndRulesDefaults.roundToDigits),
                );
            }
